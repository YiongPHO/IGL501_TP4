	...
	if ( pCamera->AvecLumiere() && pCamera->LumiereActiveScene() && Settings()->PolygoneRemplit() )
	{
		...
		for(int i = 0; i < nbLum ; i++)
		{
			...
			if ( Lum.Active() )
			{
				...
				GLfloat lightPos[] = { p2.x(),p2.y(),p2.z(), 1};

				Couleur ambiante = Lum.Ambiante();
				float ambientLight[] = {ambiante.rouge(), ambiante.vert(), ambiante.bleu(), 1.0f};
				Couleur diffuse = Lum.Diffuse();
				float diffuseLight[] = {diffuse.rouge(), diffuse.vert(), diffuse.bleu(), 1.0f};
...
{
	float v[4] = { c.rouge(), c.vert(), c.bleu(), 1.0 };
